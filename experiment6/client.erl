...
start(ClientID, Entries, Reads, Writes, Server, Percentage) ->
    NumberRandomEntries = round(Entries * Percentage),
	ListIntegers = lists:seq(1, Entries),
	RandomSelectEntries = lists:sublist([X || {_ , X} <- lists:sort([{rand:uniform(), E} || E <- ListIntegers])], NumberRandomEntries),
	spawn(fun() -> open(ClientID, RandomSelectEntries, Reads, Writes, Server, 0, 0) end).
